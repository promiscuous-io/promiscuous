  ...
  context 'when (create|update)/destroy are processed out of order' do
    ...
    context 'when consistency is eventual' do
      ...
      context 'within the timeout' do
        before do
          Promiscuous::Subscriber::Worker::EventualDestroyer.stubs(:destroy_timeout).returns(3.seconds)
      ...
      context 'not within the timeout' do
        before do
          Promiscuous::Subscriber::Worker::EventualDestroyer.stubs(:destroy_timeout).returns(10.seconds)
          Promiscuous::Subscriber::Worker::EventualDestroyer.stubs(:check_every).returns(1.second)
          @worker.eventual_destroyer.stop
          @worker.eventual_destroyer.start
        end
